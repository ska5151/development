...
VideoDB 捕获会话的代码级详情。工作流程指南请参阅 [capture.md](everything-claude-code_sample/docs/zh-CN/skills/videodb/reference/capture.md)。
...
使用 [scripts/ws\_listener.py](ws_listener.py) 连接并将事件转储到 `${VIDEODB_EVENTS_DIR:-$HOME/.local/state/videodb}/videodb_events.jsonl`。
...
| RTStream 方法 | 参见 [rtstream-reference.md](everything-claude-code_sample/docs/zh-CN/skills/videodb/reference/rtstream-reference.md) |
...
关于 RTStream 方法（索引、转录、警报、批处理配置），请参阅 [rtstream-reference.md](everything-claude-code_sample/docs/zh-CN/skills/videodb/reference/rtstream-reference.md)。
